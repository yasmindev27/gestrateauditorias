...
import logoGestrategicUrl from '@/assets/logo-gestrategic.jpg';

export const PDF_LGPD_TEXT =
  'Este relatório contém dados tratados em conformidade com a Lei Geral de Proteção de Dados (Lei nº 13.709/2018), Arts. 7º, 11 e 52. ' +
  'O conteúdo é estritamente confidencial e destinado apenas ao uso autorizado, sujeito às sanções do Art. 52 em caso de vazamento.';
...
const loadLogo = (callback: (img: HTMLImageElement | null) => void) => {
  ...
  logoImg.src = logoGestrategicUrl;
  logoImg.onload = () => callback(logoImg);
  logoImg.onerror = () => callback(null);
};

export const loadPdfLogo = (): Promise<HTMLImageElement | null> => {
...
export const applyPdfHeaderFooter = (doc: jsPDF, title: string, logoImg: HTMLImageElement | null) => {
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    const splitLgpd = doc.splitTextToSize(PDF_LGPD_TEXT, pageWidth - 28);
...
export const applyPdfFooterOnly = (doc: jsPDF) => {
  const pageWidth = doc.internal.pageSize.width;
  const pageHeight = doc.internal.pageSize.height;
  const pageCount = doc.getNumberOfPages();
...
export const exportToPDF = (options: ExportOptions): void => {
  ...
  const renderPDF = (logoImg: HTMLImageElement | null) => {
    const logoLoaded = Boolean(logoImg);
    // --- HEADER ---
    // Left: title + date
    doc.setFontSize(16);
    doc.setFont('helvetica', 'bold');
