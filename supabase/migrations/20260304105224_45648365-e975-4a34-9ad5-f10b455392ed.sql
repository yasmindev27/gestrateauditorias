...
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='auditoria_temporalidade') THEN
    DROP POLICY IF EXISTS "Usuarios autenticados podem inserir registros de temporalidade" ON public.auditoria_temporalidade;
    EXECUTE 'CREATE POLICY "Auth insert temporalidade" ON public.auditoria_temporalidade FOR INSERT TO authenticated WITH CHECK (auth.uid() IS NOT NULL)';
  END IF;
END $$;

-- justificativas_atraso
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='justificativas_atraso') THEN
    DROP POLICY IF EXISTS "Usuarios autenticados podem inserir justificativas" ON public.justificativas_atraso;
    EXECUTE 'CREATE POLICY "Auth insert justificativas" ON public.justificativas_atraso FOR INSERT TO authenticated WITH CHECK (auth.uid() IS NOT NULL)';
  END IF;
END $$;
